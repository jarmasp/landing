...
const VF_MODULES: VFModule[] = [
  { name: 'crypto-utils',    desc: 'secp256k1 elliptic curve primitives, Ethereum canonical signing, key-to-address derivation. Published as @void-foundry/crypto-utils.' },
  { name: 'hd-wallet-core',  desc: 'BIP-32/44 hierarchical deterministic wallets from scratch, cross-validated against MetaMask. xpub watch-only, safe derive CLI.' },
  { name: 'tx-indexer',      desc: 'NestJS blockchain indexer. TypeORM entities, RabbitMQ event bus, migration-ready DataSource, full Docker Compose dev stack.' },
  { name: 'smart-wallet',    desc: 'EVM smart wallet contracts on Hardhat. On-chain verification, contract test harness, semver-exposed version view.' },
  { name: 'observability',   desc: 'Self-hosted Grafana + Prometheus + Loki + Tempo. Zero SaaS cost. OTLP receiver, smoke tests, governance tooling.' },
]
            ...
            <div className="void-modules">
              {VF_MODULES.map((m) => (
                <div key={m.name} className="void-module">
                  <span className="void-module-name">{m.name}</span>
